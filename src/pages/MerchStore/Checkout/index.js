...
const Checkout = ({
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values) => {
      const fullAddress = `${values.address}, ${values.brgy}, ${values.city} ${values.postalCode}`;
      const coordinates = await getCoordinatesFromOpenCage(fullAddress);

      await props.actionCreator({
        type: types.SET_SHIP_ADDRESS_COORDINATE,
        payload: { lng: String(coordinates.lng), lat: String(coordinates.lat) },
      });
  ...
  React.useEffect(() => {
    console.log("checkout.quotations", checkout.quotations);
    checkout.quotations &&
      computeSubTotalAndShippingRate(cart.data, checkout.quotations);
  }, [checkout.quotations]);
  ...
  const computeSubTotalAndShippingRate = async (data, quotations) => {
    ...
    if (selectedOption === "for-delivery") {
      ...
      if (quotations?.priceBreakdown?.total) {
        const shippingRate = Number(quotations?.priceBreakdown?.total);

        setShippingTotalRate(shippingRate);
        const newTotalAmount =
          parseFloat(TotalAmount) + parseFloat(shippingRate);
  ...
  const placeOrder = async () => {
    if (selectedPaymentOption) {
      ...
      let ShippingAddress = "";
      let QuotationID = "";
      let RecipientShippingStopID = "";
      let SenderShippingStopID = "";
      let formattedNumber = "";
      if (checkout.shipMethod === "For Pickup") {
        ...
        await props.actionCreator({
          ...
          payload: {
            ...
            CustomerName: checkout.shipAddressData[0]?.fullName || "Customer",
            ...
            Long: checkout.shipAddressCoordinates?.lng || "",
            Lat: checkout.shipAddressCoordinates?.lat || "",
            QuotationID: QuotationID,
            RecipientShippingStopID: RecipientShippingStopID,
            SenderShippingStopID: SenderShippingStopID,
          },
        });
      } else {
        ShippingAddress = `${checkout.shipAddressData[0]?.fullName || "Customer"}, ${checkout.shipAddressData[0].address}, ${checkout.shipAddressData[0].brgy}, ${checkout.shipAddressData[0].postalCode}, ${checkout.shipAddressData[0].city}, ${checkout.shipAddressData[0].telephone}`;
        QuotationID = checkout.quotations?.quotationId;
        RecipientShippingStopID = checkout.quotations?.stops
          ? checkout.quotations?.stops[1]?.stopId
          : "";
        SenderShippingStopID = checkout.quotations?.stops
          ? checkout.quotations?.stops[0]?.stopId
          : "";
        formattedNumber = checkout.shipAddressData[0]?.telephone.replace(
          /^0/,
          "+63"
        );

        if (SenderShippingStopID) {
          await props.actionCreator({
            type: types.PLACE_ORDER,
            payload: {
              Description: description,
          ...
          Swal.fire({
            icon: "error",
            title: "Shipping Lalamove",
            text: "Delivery not available right now, Please select Picku-up option.",
          });
        }
      }
    } else {
  ...
  const getCoordinatesFromOpenCage = async (address) => {
    const apiKey = "d9adf3a925924e658b036e9703b200b8"; // Replace this with your real key
    const encodedAddress = encodeURIComponent(address);
    const url = `https://api.opencagedata.com/geocode/v1/json?q=${encodedAddress}&key=${apiKey}&language=en&pretty=1&countrycode=ph`;
  ...
  //meta title
  document.title = "Seoulful Sweets | Checkout";
            ...
            <div style={{ width: 530, margin: 5 }}>
              <div className="checkout-details h-full hidden md:block">
                ...
                <div className="checkout-details-block">
                  ...
                  {selectedOption === "for-delivery" && shippingTotalRate && (
                    <div className="checkout-details-block__total">
                      <h5>Shipping Fee</h5>
